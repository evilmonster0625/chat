...
//        resp.setTotalNum(onlineNum + offlineNum);
    ...
    @Transactional
    @Override
    public void secureTest(ChatMessageReq chatMessageReq) {
        Message message = Message.builder()
                .fromUid(1L)
                .roomId(2L)
                .status(1)
                .content(chatMessageReq.getBody().toString())
                .createTime(new Date())
                .updateTime(new Date())
                .build();
        messageDao.save(message);

        List<Message> collect = Stream.of(message).collect(Collectors.toList());
        chatService.secList(collect);
//        chatService.sec(chatMessageReq.getRoomId(),chatMessageReq.getMsgType(),chatMessageReq.getBody());
    }

    @SecureInvoke
    public void sec(Long roomId,Integer msgType,Object body){
    ...
    @SecureInvoke
    public void secList( List<Message> collect){
        collect.forEach(x -> System.out.println(x.getFromUid()));
    }
